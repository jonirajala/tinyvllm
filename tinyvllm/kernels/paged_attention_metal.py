...
class PagedAttentionMetal:
    ...
    def _ensure_compiled(self):
        ...
        lib = compiler.compile(PAGED_ATTENTION_KERNEL)

        PagedAttentionMetal._program_batched = MetalProgram(device, 'paged_attention_batched', lib)
        self._compiled = True
    ...
    def batched(
        ...
        # Allocate output and get Metal buffers
        ...
        v_buf = get_metal_buffer(v_cache)
        pt_buf = get_metal_buffer(Tensor(padded_tables, dtype=dtypes.int32))
        ctx_buf = get_metal_buffer(Tensor(context_lens, dtype=dtypes.int32)) # Context lengths tensor
        out_buf = get_metal_buffer(output)

        # Run batched kernel
        self._program_batched(
            q_buf._buf,
            k_buf._buf,
            v_buf._buf,
            pt_buf._buf,
